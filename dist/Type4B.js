...
export class Type4BTag {
    constructor(rcs380) {
        ...
        this.pni = 0;
        this.protocol = Uint8Array.of(0x0b, 0x01, 0x09, 0x01, 0x0c, 0x01, 0x0a, 0x01, 0x00, 0x14);
        this.rf = Uint8Array.of(0x03, 0x07, 0x0f, 0x07);
    }
    ...
    async sendAttribute() {
        ...
        attribute.set(Uint8Array.of(0x00, 0x08, 0x01, 0x00), 1 + this.nfcID.byteLength);
    ...
    async exchange(type4BCommand, timeoutMs) {
        ...
        if (Boolean(response.data[7] & 0b00010000)) {
            ...
            await this.rcs380.sendPreparationCommands(this.rf, this.protocol);
            const a2Response = await this.rcs380.inCommRf(Uint8Array.of(0xa2 | this.pni), timeoutMs);
